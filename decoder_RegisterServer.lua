function decodeRegisterServer(buffer, cursor, proto, lmp_base_subtree)
    local subtree = lmp_base_subtree:add(proto,buffer(23),"RegisterServer")

    -- MsRegisterServerMsgData / RegisterServer
    ...
    subtree:add(buffer(cursor,4),"Secondary Vessel Update Interval: " .. buffer(cursor,4):le_uint())
    cursor = cursor + 4

    subtree:add(buffer(cursor,1),"Rainbow Effect: " .. buffer(cursor,1):bitfield(7, 1))
